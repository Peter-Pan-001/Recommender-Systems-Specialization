...
public class ItemMeanModelProvider implements Provider<ItemMeanModel> {
    ...
    public ItemMeanModel get() {
        ...
        HashMap<Long, List<Rating>> itemsRatingsMap = new HashMap<>();

        try (ObjectStream<Rating> ratings = dao.query(Rating.class).stream()) {
            for (Rating r : ratings) {
                // this loop will run once for each rating in the data set
                // TODO process this rating
                if (itemsRatingsMap.containsKey(r.getItemId())) {
                    itemsRatingsMap.get(r.getItemId()).add(r);
                } else {
                    itemsRatingsMap.put(r.getItemId(), Lists.newArrayList(r));
                }
            }
        }
        ...
        for (long itemId : itemsRatingsMap.keySet()) {
            double sum = 0.0;
            for (Rating r : itemsRatingsMap.get(itemId)) {
                sum += r.getValue();
            }
            means.put(itemId, sum / itemsRatingsMap.get(itemId).size());
        }
