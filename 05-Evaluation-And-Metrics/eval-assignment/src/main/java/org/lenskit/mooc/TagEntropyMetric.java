...
public class TagEntropyMetric extends TopNMetric<TagEntropyMetric.Context> {
    ...
    public MetricResult measureUser(TestUser user, int expectedSize, ResultList recommendations, Context context) {
        ...
        for (Long itemID : recommendations.idList()){

            List<Entity> tags = dao.query(TagData.ITEM_TAG_TYPE).withAttribute(TagData.ITEM_ID, itemID).get();
            Set<Entity> seenTags = new HashSet<>();
            for(Entity tag: tags){ seenTags.add(tag); }
            int size = seenTags.size();

            for (Entity itemTag : seenTags){
                Long tagId = Long.parseLong(itemTag.get(TagData.TAG));
