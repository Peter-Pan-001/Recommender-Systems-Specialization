    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                ...
                if (work.containsKey(tag)) {
                    ...
                    work.put(tag, 1.0);
                    docFreq.put(tag, docFreq.containsKey(tag) ? docFreq.get(tag) + 1 : 1.0);
                }
            }
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            for (Map.Entry<String, Double> e : tv.entrySet()) {
                Double TFIDFValue = e.getValue() * docFreq.get(e.getKey());
                e.setValue(TFIDFValue);
                sumOfSquare += TFIDFValue * TFIDFValue;
            }
            ...
            Double norm = Math.sqrt(sumOfSquare);
            for (Map.Entry<String, Double> e : tv.entrySet()) {
