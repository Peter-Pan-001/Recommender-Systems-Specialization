...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        for (Long item: items) {
            ...
            Double itemSquareSum = 0.0;
            Double profileSquareSum = 0.0;

            for (Map.Entry<String, Double> entry : iv.entrySet()) {
                Double tagValue = entry.getValue();
                numerator += userVector.containsKey(entry.getKey()) ? (tagValue * userVector.get(entry.getKey())) : 0;
                itemSquareSum += tagValue * tagValue;
            }

            for (Double value : userVector.values()) {
                profileSquareSum += value * value;
            }
            denominator = Math.sqrt(itemSquareSum) * Math.sqrt(profileSquareSum);
