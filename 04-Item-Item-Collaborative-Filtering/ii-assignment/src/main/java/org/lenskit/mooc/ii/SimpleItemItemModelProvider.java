...
public class SimpleItemItemModelProvider implements Provider<SimpleItemItemModel> {
    ...
    public SimpleItemItemModel get() {
        ...
        for(Map.Entry<Long,Long2DoubleMap> item : itemVectors.entrySet()) {

            Long2DoubleMap itemSimilarity = new Long2DoubleOpenHashMap();

            for(Map.Entry<Long,Long2DoubleMap> neighbor : itemVectors.entrySet()) {

                if (item.getKey() == neighbor.getKey()) { continue; }

                Double cosineValue = (Vectors.dotProduct(item.getValue(), neighbor.getValue())) /
                        (Vectors.euclideanNorm(item.getValue())* Vectors.euclideanNorm(neighbor.getValue()));

                if(cosineValue > 0.0) {
                    itemSimilarity.put(neighbor.getKey(), cosineValue);
                }

            }

            itemSimilarities.put(item.getKey(), itemSimilarity);
        }
